...
export function LikeTagsPage() {
    ...
    const tagOnClick = async (tag_id, status) => {
        try {
            console.log(status);
            if (status) {
                ...
                const response = await axios.delete("http://localhost:8000/api/user/delete-like/tag/",
                ...
                console.log(response.data);
                setLikedTags(likedTags => likedTags.filter((tag)=> tag.id != tag_id ))
            }
